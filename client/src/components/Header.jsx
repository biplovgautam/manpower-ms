...
export function Header({
    ...
    const userData = useMemo(() => {
        const name = user?.fullName || user?.user?.fullName || user?.data?.fullName || user?.name || "User";
    ...
    const handleMarkAllRead = async (e) => {
        ...
        try {
            if (typeof onMarkAllRead === 'function') await onMarkAllRead();
        } catch (err) {
    ...
    const handleViewHistory = (e) => {
        e.preventDefault();
        setIsNotifOpen(false);
        // We pass the clean segment name. 
        // The Parent component should handle the base path.
                ...
                <div className="flex items-center gap-4 md:gap-6">
                        ...
                        {isNotifOpen && (
                            <div className="absolute right-0 mt-3 w-80 sm:w-96 bg-white border border-slate-200 shadow-2xl rounded-2xl overflow-hidden animate-in fade-in zoom-in duration-150 origin-top-right">
                                <div className="p-4 border-b flex justify-between items-center bg-slate-50/70">
                                    ...
                                    {unreadNotifications.length > 0 && (
                                        <button onClick={handleMarkAllRead} disabled={isMarkingRead} className="flex items-center gap-1.5 text-xs font-bold text-indigo-600 hover:text-indigo-800">
                                            {isMarkingRead ? <Loader2 size={14} className="animate-spin" /> : <CheckCheck size={14} />} Mark all read
                                        </button>
                                    )}
                                </div>
                                ...
                                <div className="max-h-[340px] overflow-y-auto divide-y divide-slate-100">
                                    {unreadNotifications.length > 0 ? (
                                        unreadNotifications.map((item) => (
                                            <div key={item._id} className="p-4 hover:bg-slate-50/50 transition-colors">
                                        ...
                                        <div className="p-10 text-center text-slate-400 italic text-sm">You're all caught up!</div>
                                    )}
                                </div>

                                <button onClick={handleViewHistory} className="w-full p-3.5 bg-slate-50 border-t border-slate-100 text-center text-xs font-bold text-slate-500 hover:text-indigo-600 flex items-center justify-center gap-2 uppercase">
                    ...
                    <div className="flex items-center gap-3 pl-1">
                        <div className="text-right hidden sm:block">
                            ...
                            <p className="text-[10px] text-indigo-600 font-bold uppercase">{userData.role.replace(/_/g, ' ')}</p>
                        </div>
                        <div className="w-10 h-10 bg-indigo-600 rounded-xl flex items-center justify-center text-white font-bold shadow-lg cursor-pointer hover:scale-105 transition-transform" onClick={onLogout}>
                            {userData.avatar}
                        </div>
                    </div>
                </div>
